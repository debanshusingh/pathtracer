...
inline glm::vec3 getRandomPointOnSphere(Node* sphere) {
	...
	return vec3(sphere->getTransformation()*vec4(point,1));
}

// Returns a random point on a cube. Adapted from CIS 565
inline glm::vec3 getRandomPointOnCube(Node* &cubeLight) {
	// TODO: get the dimensions of the transformed cube in world space
	mat4 transform = cubeLight->getGlobalTransformation();
	glm::vec3 dim = vec3(transform[0][0], transform[1][1], transform[2][2]);
	...
	// TODO: transform point to world space
	vec4 lightPos = transform*vec4(point,1);
	if (lightPos[0] != 0) {		
		lightPos /= lightPos[3];
	}
	return vec3(lightPos);
}

// Given a normal vector, find a cosine weighted random direction in a hemisphere
// Adapted from CIS 565
inline glm::vec3 getCosineWeightedDirection(const glm::vec3& normal) {
